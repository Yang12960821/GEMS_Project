...
file_path = ("C:/Users/hp/Desktop/GEMS/wang20220316173017/")
#npp_nc = Dataset(file_path + "/ncproda_cgcm_yrb1km_rcp26.nc4")
npp_nc = Dataset(file_path + "/somsc_cgcm_yrb1km_rcp26.nc4")
#npp_data = npp_nc.variables['ncproda'][:].data
npp_data = npp_nc.variables['somsc'][:].data
npp_data[npp_data<10] = npp_data[0,0,0]
npp_data[npp_data==-9999] = np.nan

if os.path.exists(file_path+'NPP') != 1:
    os.mkdir(file_path+'NPP')
    
if os.path.exists(file_path+'NPP/2000_2015') != 1:
    os.mkdir(file_path+'NPP/2000_2015')
    
if os.path.exists(file_path+'NPP/2000_2050') != 1:
    os.mkdir(file_path+'NPP/2000_2050')
    
if os.path.exists(file_path+'NPP/2000_20501') != 1:
    os.mkdir(file_path+'NPP/2000_20501')    

if os.path.exists(file_path+'NPP/2035_2050') != 1:
    os.mkdir(file_path+'NPP/2035_2050')
    
fig,ax = plt.subplots(4,4,figsize=(5.5,4),sharex=False, sharey=False)
for i in range(4):
    for j in range(4):
        im = ax[i][j].contourf(x,y,npp_data[i*4+j+8],cmap = 'RdYlGn_r',levels=np.arange(0,3000.1,500),extend='both')
        ax[i][j].axis('off')
        ax[i][j].text(102,41,str(1992+8+j+4*i),fontsize=6)
              
plt.subplots_adjust(wspace=-0.4,hspace=-0.11)
fig.colorbar(im, ax=[ax[0][0],ax[0][1],ax[0][2],ax[0][3],ax[1][0],ax[1][1],ax[1][2],ax[1][3],ax[2][0],ax[2][1],ax[2][2],ax[2][3],ax[3][0], ax[3][1],ax[3][2],ax[3][3]],fraction=0.04, pad = 0,orientation = 'horizontal',label='NPP (g C m$^-$$^2$ yr$^-$$^1$)',extend='both')
plt.savefig(file_path + "npp1/2000_2015/2000_2015.jpg",dpi = 1000)
...
for i in range(0,59,5):
    ...
    im = plt.contourf(x,y,npp_data[i],cmap = cmap,levels=np.arange(0,3000.1,500),extend='both')
    plt.text(102,40,str(1992+i),fontsize=16)
    plt.axis('off')
    plt.colorbar(im,fraction = 0.04, pad = - 0.12,orientation = 'horizontal',label='NPP (g C m$^-$$^2$ yr$^-$$^1$)',extend='both')
#     plt.show()
    plt.savefig(file_path + "npp/2000_2050n/npp_" + str(1992+i)+ ".jpg",dpi = 400,bbox_inches='tight')
...
# import cv2
#指明被遍历的文件夹
rootdir = r'C:\Users\hp\Desktop\gems\wang20220316173017\npp\2000_2050n'
for parent, dirnames, filenames in os.walk(rootdir):#遍历每一张图片
    for filename in filenames:
        ...
        image1.save(r"C:\Users\hp\Desktop\gems\wang20220316173017\npp\2000_20501n"+'\\'+filename) #存储裁剪得到的图像
...
def main():
    ...
    path = file_path + "npp/2000_20501/"
    a = [ path+img for img in  os.listdir(path)]
    image_list = []
    for name in a:
#         if (name[-5] == str(0))|(name[-5] == str(5)):
        image_list.append(name)
    gif_name = file_path + "npp/2000_20501/npp.gif"
...
for i in range(4):
    for j in range(4):
        im = ax[i][j].contourf(x,y,npp_data[i*4+j+8+35],cmap = 'RdYlGn_r',levels=np.arange(0,900.1,150),extend='both')
        ax[i][j].axis('off')
        ax[i][j].text(102,41,str(1992+8+j+4*i+35),fontsize=6)
              
plt.subplots_adjust(wspace=-0.4,hspace=-0.11)
fig.colorbar(im, ax=[ax[0][0],ax[0][1],ax[0][2],ax[0][3],ax[1][0],ax[1][1],ax[1][2],ax[1][3],ax[2][0],ax[2][1],ax[2][2],ax[2][3],ax[3][0], ax[3][1],ax[3][2],ax[3][3]],fraction=0.034, pad = 0,orientation = 'horizontal',label='NPP (g C m$^-$$^2$ yr$^-$$^1$)',extend='both')
plt.savefig(file_path + "npp/2035_2050/2035_2050.jpg",dpi = 1000)
