...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(soil_layers_top)>1:
		data_variable=mydataset.createVariable(data_variable_name,np.ubyte,('soil_layers_top','y','x'),fill_value=255)
	else:
	...
	for i in range(len(soil_layers_top)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata!=NoDataValue]=80
...
if __name__ == '__main__':
	#以土壤深度数据作为代替栅格范围数据，将所有的值改为80
	input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/soil/soil_thick/YRB1km/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/2.soil/11.soil_drainage/soil_drainage_yrb1km_dv01.nc4"
	data_variable_name='drainage'
	data_variable_longname="soil component percentages for drainage classes"
	all_vars_units=['cm','meters','meters','percent']
	#begin_end_date=["0cm","200cm"]
	begin_end_date=["1992","2015"]
	#Fill_value=-3.40282346639e+038
	filetitle="Gridded Soil Drainage"
	Scale_factor=1
