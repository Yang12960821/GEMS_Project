...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(final_file_list)>1:
		soil_layers_top=mydataset.createVariable('soil_layers_top','u2',('soil_layers_top',))
		soil_layers_bottom=mydataset.createVariable('soil_layers_bottom','u2',('soil_layers_bottom',))
		data_variable=mydataset.createVariable(data_variable_name,'u2',('soil_layers_top','y','x'),fill_value=np.ushort(255))
		#给土壤层赋属性
		...
		soil_layers_bottom.standard_name='soil_layers_bottom'
		...
		data_variable=mydataset.createVariable(data_variable_name,'u2',('y','x'),fill_value=np.ushort(255))
	#对变量的单位进行设定
	#soil_layers_top.units=all_vars_units[0]
	#soil_layers_bottom.units=all_vars_units[0]
...
input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/soil/soil_thick/0土壤深度数据更新/"
input_rasterfiles_type='.tif'
output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/2.soil/6.soil_thick_updated/soil_thick_yrb1km_dv01.nc4"
data_variable_name='thick_tp'
data_variable_longname="soil thickness"
all_vars_units=['cm','meters','meters','cm']
#begin_end_date=["0cm","200cm"]
begin_end_date=["1992","2015"]
#Fill_value=-3.40282346639e+038
filetitle="Overall Soil Thickness"
Scale_factor=1
