...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(soil_layers_top)>1:
		data_variable=mydataset.createVariable(data_variable_name,'f4',('soil_layers_top','y','x'),fill_value=Fill_value)
	else:
	...
	for i in range(len(soil_layers_top)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata!=NoDataValue]=rasterdata[rasterdata!=NoDataValue]
...
if __name__ == '__main__':
	#AWC是小数(原始单位为百分之)
	##GEMS_CON_Unit4.cpp文件中注明：采用的AWW参考文献为：Kern, Jeffrey S . Evaluation of Soil Water Retention Models Based on Basic Soil Physical Properties[J]. Soil Science Society of America Journal, 1995, 59(4):1134.
	input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/soil/awc/0_updated_awc/awc_小数/6层数据/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/2.soil/7.soil_awc_updated/soil_awc_yrb1km_dv01.nc4"
	data_variable_name='awc'
	data_variable_longname="available water capacity"
	all_vars_units=['cm','meters','meters','cm/cm']
	#begin_end_date=["0cm","200cm"]
	begin_end_date=["1992","2015"]
	#Fill_value=-3.40282346639e+038
	filetitle="Gridded AWC for 6 Soil Layers"
	Scale_factor=1
