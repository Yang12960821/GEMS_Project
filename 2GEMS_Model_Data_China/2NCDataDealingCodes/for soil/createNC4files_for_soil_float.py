...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	for i in range(len(soil_layers_top)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata!=NoDataValue]=rasterdata[rasterdata!=NoDataValue]*1000.0
...
if __name__ == '__main__':
	input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/soil/soil_soc/0_updated_YRB_soil_calculated/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/2.soil/5.soil_soc_updated/soil_soc_yrb1km_dv01.nc4"
	data_variable_name='soc'
	data_variable_longname="soil organic carbon"
	all_vars_units=['cm','meters','meters','gC/m^2']
	#begin_end_date=["0cm","200cm"]
	begin_end_date=["1992","2015"]
	#Fill_value=-3.40282346639e+038
	filetitle="Gridded SOC for 6 Soil Layers"
	Scale_factor=1
