...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(soil_layers_top)>1:
		data_variable=mydataset.createVariable(data_variable_name,'u2',('soil_layers_top','y','x'),fill_value=65535)
	else:
		data_variable=mydataset.createVariable(data_variable_name,'u2',('y','x'),fill_value=65535)
	#对变量的单位进行设定
	...
	for i in range(len(soil_layers_top)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata!=NoDataValue]=rasterdata[rasterdata!=NoDataValue]*1000
...
input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/soil/soil_kfactor/0_kfactor_updated\YRB_mean"
input_rasterfiles_type='.tif'
output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/2.soil/8.soil_kfactor_updated/soil_kfactor_yrb1km_dv01.nc4"
data_variable_name='kfactor'
data_variable_longname="soil erodibility factor"
all_vars_units=['cm','meters','meters','fraction']
#begin_end_date=["0cm","200cm"]
begin_end_date=["1992","2015"]
#Fill_value=-3.40282346639e+038
filetitle="Soil Erodibility Factor"
Scale_factor=0.001
