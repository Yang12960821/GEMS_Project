...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	#构建维度
	mydataset.createDimension('soil_layers',len(final_file_list))
	...
	#构建变量
	soil_layers=mydataset.createVariable('soil_layers',np.int16,('soil_layers',))
	...
	if len(soil_layers)>1:
		data_variable=mydataset.createVariable(data_variable_name,np.float32,('soil_layers','y','x'),fill_value=Fill_value)
	else:
		data_variable=mydataset.createVariable(data_variable_name,np.float32,('y','x'),fill_value=Fill_value)
	#对变量的单位进行设定
	soil_layers.units=all_vars_units[0]
	lats.units=all_vars_units[1]
	lons.units=all_vars_units[2]
	data_variable.units=all_vars_units[3]
	#变量的相关参数
	soil_layers.standard_name='soil_layers'
	soil_layers.long_time='soil_layers'
	soil_layers.axis='soil_layers'
	...
	for i in range(len(soil_layers)):
		...
		print 'progress'+str(round((i+1)*100.0/len(soil_layers),2))+"%"
	...
	mydataset.setncattr('begin_date_(yyyy-mm)','198')
	mydataset.setncattr('end_date_(yyyy-mm)',begin_end_date[0])
	mydataset.setncattr('begin_date_(yyyy)', begin_end_date[1])
...
input_rasterfiles_path=u"H:/GEMS_Model_data/my_studyarea_data/raster_data/soil/soil_sand/YRB1km"
input_rasterfiles_type='.tif'
output_NC_file=u"H:/GEMS_Model_data/my_studyarea_data/nc4_data/soil/soil_sand/soil_sand_yrb1km_dv01.nc4"
data_variable_name='sand_ttl'
data_variable_longname='soil erodibility factor'
all_vars_units=['','meters','meters','cm']
#begin_end_date=["0cm","200cm"]
begin_end_date=["",""]
#Fill_value=-3.40282346639e+038
filetitle="Gridded Total Sand for 6 Soil Layers"
Scale_factor=1
