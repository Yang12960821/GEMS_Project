...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(final_file_list)>1:
		...
		data_variable=mydataset.createVariable(data_variable_name,'u2',('time','y','x'),fill_value=0)
		...
		data_variable=mydataset.createVariable(data_variable_name,'u2',('y','x'),fill_value=0)
	#对变量的单位进行设定
...
#input 1 file 只是为了获取数据范围
input_rasterfiles_path=u"I:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/reference/YRB_1km_reference_based_CHN1km/FIPS/"
input_rasterfiles_type='.tif'
output_NC_file=u"I:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/7.reference/NEW_reference_based_CHN1km/reference_fips_yrb1km_dv01.nc4"
data_variable_name='county_id'
data_variable_longname="5-digit state-county FIPS ID"
all_vars_units=['years since 2016-12','meters','meters','years']
#begin_end_date=["0cm","200cm"]
begin_end_date=["2016","2050"]
#Fill_value=-3.40282346639e+038
filetitle="State-County FIPS ID"
Scale_factor=1
