	...
	for each in pathDir:
		...
		if os.path.isfile(newDir):
			if os.path.splitext(newDir)[1]==filetype:
				final_file_list.append(newDir)
		else:
			read_all_files_inside_filepath(newDir,filetype,final_file_list)

#reading the file pathes of the specific datatype
#returen a list containing all paths of these data
def get_type_filepath(filepath,filetype):
	final_file_list=[]
...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	final_file_list=get_type_filepath(input_rasterfiles_path,input_rasterfiles_type)[120:1140]
	...
	for i in range(len(times)):
		...
		print_current_process(round((i+1)*1.0/len(times),4))
...
if __name__ == '__main__':
	for gcm in ['CSIRO-Mk3.6']:
		for rcp in ['rcp26','rcp45','rcp85']:
			input_rasterfiles_path=u"B:/Data/6_GEMs未来气候数据delta降尺度/5CGCMs矫正降尺度_黄河流域2006_2010/"+gcm+"/tasmax/"+rcp+"/"
			input_rasterfiles_type='.tif'
			output_NC_file=u"B:/Data/6_GEMs未来气候数据delta降尺度/7YRB_Climate_NC/monthly_tmax_degC/Futuretmax2016_2100/"+gcm+"/"+rcp+"/climate_tmax_yrb1km_"+gcm+"_"+rcp+"_2016_2100_dv02.nc4"						
			data_variable_name='tmax'
			data_variable_longname='average maximum temperature'
			all_vars_units=['months since 2016','meters','meters','degrees C']
			begin_end_date=["2016-01","2100-12"]
			#Fill_value=0
			filetitle='Monthly Average Maximum Temperature'
			Scale_factor=1
