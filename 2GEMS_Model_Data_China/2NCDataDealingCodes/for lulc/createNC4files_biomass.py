...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(times)>1:
		data_variable=mydataset.createVariable(data_variable_name,np.float32,('time','y','x'),fill_value=Fill_value)
	else:
...
#针对单个NC文件单个变量的数据
input_rasterfiles_path=u"H:/GEMS_Model_data/my_studyarea_data/raster_data/lucc/biomass/shrubland_biomass/Biomass_based_FVC/MEAN_BIOMASS/YRB_TB/"
input_rasterfiles_type='.tif'
output_NC_file=u"H:/GEMS_Model_data/my_studyarea_data/nc4_data/lucc/Shurb_totbiom/YRB_shurb_totbiom_1km_dv1.0.nc4"
data_variable_name='shrub_totbiom'
data_variable_longname='shrubland total biomass'
all_vars_units=['months since 2000','meters','meters','gC/m^2']
begin_end_date=["2000","2015"]
filetitle='Shrubland/Grassland Biomass'
