...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	final_file_list=final_file_list1*35
	#降雨、温度需要排序
	...
	if len(times)>1:
		...
		data_variable=mydataset.createVariable(data_variable_name,'u2',('time','y','x'))
	else:
		#data_variable=mydataset.createVariable(data_variable_name,np.int16,('y','x'),fill_value=np.ubyte(Fill_value))
		data_variable=mydataset.createVariable(data_variable_name,'u2',('y','x'))
	#对变量的单位进行设定
	...
	for i in range(len(times)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata!=0]=rasterdata[rasterdata!=0]+i+24
...
if __name__ == '__main__':
	input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/forest/forest_age/1992_2015/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/6.forest/2.forest_ages/forest_standage_yrb1km_2016_2050_dv01.nc4"
	data_variable_name='stand_age'
	data_variable_longname="forest stand age"
	...
	filetitle="Forest Stand Age Data"
	Scale_factor=1
