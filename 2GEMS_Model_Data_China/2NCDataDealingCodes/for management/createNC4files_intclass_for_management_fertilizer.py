...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(times)>1:
		...
		data_variable=mydataset.createVariable(data_variable_name,'u2',('time','y','x'),fill_value=Fill_value)
	else:
		#data_variable=mydataset.createVariable(data_variable_name,np.int16,('y','x'),fill_value=np.ubyte(Fill_value))
		data_variable=mydataset.createVariable(data_variable_name,'u2',('y','x'),fill_value=Fill_value)
	#对变量的单位进行设定
...
input_rasterfiles_path=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/management/fertilizer/0YRB_Fertilizer_gN_m2_1km_update/"
input_rasterfiles_type='.tif'
output_NC_file=u"F:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/4.management/6.fertilizer/mgmt_fertilizer_yrb1km_1992_2015_dv01.nc4"
