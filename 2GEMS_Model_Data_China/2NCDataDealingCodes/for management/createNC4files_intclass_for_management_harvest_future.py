...
def convert_nclistfiles_to_raster(nc_files_list,nc_variables,output_rasterfile_path,im_width,im_height,im_bands,im_geotrans,im_proj):
	...
	for each in nc_files_list:		
		#print each.replace("\\","/")
		print('第', str(nc_files_list.index(each) + 1), '开始...')
		starttime=time.time()
...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle):
	...
	if len(times)>1:
		...
		data_variable=mydataset.createVariable(data_variable_name,np.ubyte,('time','y','x'),fill_value=255)
	else:
		#data_variable=mydataset.createVariable(data_variable_name,np.int16,('y','x'),fill_value=np.ubyte(Fill_value))
		data_variable=mydataset.createVariable(data_variable_name,np.ubyte,('y','x'),fill_value=255)
	#对变量的单位进行设定
	...
	#data_variable.flag_values=np.arange(1,4,1).astype(np.ubyte)
	#data_variable.flag_values=np.arange(1,18,1)
	#data_variable.flag_meanings='no_cover_crop, 1:maize, 2:rice, 3:soybean, 4:wheat'
	...
	mydataset.setncattr('upleft(x,y) center',[-2638495.7,5921435.2])
	mydataset.setncattr('pixsize(x,y)',[1000.0,1000.0])
	mydataset.close()


if __name__ == '__main__':
	input_rasterfiles_path=u"G:/GEMS数据/2GEMS_Model_Data_China/1Data/2Management/crop_harvest/2016_2050/RCP8.5/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"G:/GEMSupleft/2GEMS_Model_Data_China/1Data/NC4files/Management/harvested/mgmt_crop_harvest_chn1km_cgcm_rcp85_2016_2050_dv01.nc4"
	data_variable_name='harvest'
	data_variable_longname='crop harvested'
	all_vars_units=['years since 2016-12','meters','meters','true or false']
	begin_end_date=["2016","2050"]
	#Fill_value=0
	filetitle="Land Carbon Derived Crop Harvested Dataset"
	Scale_factor=1.0E-5
