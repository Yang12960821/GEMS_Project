...
def create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle,YRB_mask_raster_file):
	final_file_list=[]
	read_all_files_inside_filepath(input_rasterfiles_path,input_rasterfiles_type,final_file_list)
	#读取LP mask文件
	YRB_mask=read_rasterfile(YRB_mask_raster_file)[0]
	#YRB_mask=YRB_mask_data
	...
	for i in range(len(times)):
		if data_variable.ndim==2:
			...
			rasterdata[rasterdata==NoDataValue]=0
			rasterdata[YRB_mask==0]=0
	...
	mydataset.history= 'management crop type data created at' + time.ctime(time.time())+'. Based on the global crop harvested fraction data with a resolution of 10km'
	#mydataset.history= 'Last created at ' + time.ctime(time.time())+'. LULC data was created from the ESACCI global landcover data with a spatial resolution of 300m'
	#mydataset.datainfo='LULC codes range from 1 to 17 with meanings referencing to the data-flag. Notice that there exists missing codes from 3 to 6 and 14.'
...
if __name__ == '__main__':
	input_rasterfiles_path=u"I:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/raster_data/management/mgmt_croptype/更新的YRB作物类型/lulcAndCropType1992_2015/"
	input_rasterfiles_type='.tif'
	output_NC_file=u"I:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/4.management/1.mgmt_crop_type_and_lulc_update/Only_YRB/mgmt_crop_type_yrb1km_1992_2015_dv01_YRB.nc4"
	data_variable_name='crop'
	YRB_mask_raster_file=u'I:/2GEMS/1GEMS_Model_Data_YRB/1YRB_data/nc4_data/4.management/1.mgmt_crop_type_and_lulc_update/Only_YRB/yrb_mask_1_0.tif'
	data_variable_longname='landcover and crop type ID'
	all_vars_units=['years since 1992','meters','meters','non-ag landcover ID and crop-type ID for index in represented_crops with offset of 100']
	begin_end_date=["1992","2015"]
	#Fill_value=0
	#1-17 except 13 represents LULC with code 13 replaced by crop type adding an offset of 100
	Scale_factor=1
	filetitle='Land Carbon Derived Crop Type'
	create_NC_by_raster(input_rasterfiles_path,input_rasterfiles_type,output_NC_file,data_variable_name,data_variable_longname,all_vars_units,Scale_factor,begin_end_date,filetitle,YRB_mask_raster_file)
