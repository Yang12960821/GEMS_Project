...
npp_nc = Dataset(file_path + "/fsysc_cgcm_yrb1km_rcp26.nc4")
#npp_data = npp_nc.variables['ncproda'][:].data
npp_data = npp_nc.variables['fsysc'][:].data
npp_data[npp_data<10] = npp_data[0,0,0]
npp_data[npp_data==-9999] = np.nan

if os.path.exists(file_path+'FSYSC') != 1:
    os.mkdir(file_path+'FSYSC')
    
if os.path.exists(file_path+'FSYSC/2000_2015') != 1:
    os.mkdir(file_path+'FSYSC/2000_2015')
    
if os.path.exists(file_path+'FSYSC/2000_2050') != 1:
    os.mkdir(file_path+'FSYSC/2000_2050')
    
if os.path.exists(file_path+'FSYSC/2000_20501') != 1:
    os.mkdir(file_path+'FSYSC/2000_20501')    

if os.path.exists(file_path+'FSYSC/2035_2050') != 1:
    os.mkdir(file_path+'FSYSC/2035_2050')
    
if os.path.exists(file_path+'FSYSC/2000_2050n') != 1:
    os.mkdir(file_path+'FSYSC/2000_2050n')
...
fig.colorbar(im, ax=[ax[0][0],ax[0][1],ax[0][2],ax[0][3],ax[1][0],ax[1][1],ax[1][2],ax[1][3],ax[2][0],ax[2][1],ax[2][2],ax[2][3],ax[3][0], ax[3][1],ax[3][2],ax[3][3]],fraction=0.047, pad = 0,orientation = 'horizontal',label='FSYSC (g C m$^-$$^2$)',extend='both')
plt.savefig(file_path + "FSYSC/2000_2015/2000_2015.jpg",dpi = 1000)
...
for i in range(0,59,5):
    ...
    plt.axis('off')
    plt.colorbar(im,fraction = 0.045, pad = - 0.12,orientation = 'horizontal',label='FSYSC (g C m$^-$$^2$)',extend='both')
#     plt.show()
    plt.savefig(file_path + "FSYSC/2000_2050/fsysc_" + str(1992+i)+ ".jpg",dpi = 400,bbox_inches='tight')
...
# import cv2
#指明被遍历的文件夹
rootdir = r'C:\Users\hp\Desktop\wang\FSYSC\2000_2050'
for parent, dirnames, filenames in os.walk(rootdir):#遍历每一张图片
    for filename in filenames:
        ...
        image1.save(r"C:\Users\hp\Desktop\wang\FSYSC\2000_2050n"+'\\'+filename) #存储裁剪得到的图像
...
def main():
    ...
    path = file_path + "FSYSC/2000_2050n/"
    a = [ path+img for img in  os.listdir(path)]
    image_list = []
    for name in a:
#         if (name[-5] == str(0))|(name[-5] == str(5)):
        image_list.append(name)
    gif_name = file_path + "FSYSC/2000_20501/fsysc.gif"
...
plt.subplots_adjust(wspace=-0.4,hspace=-0.11)
fig.colorbar(im, ax=[ax[0][0],ax[0][1],ax[0][2],ax[0][3],ax[1][0],ax[1][1],ax[1][2],ax[1][3],ax[2][0],ax[2][1],ax[2][2],ax[2][3],ax[3][0], ax[3][1],ax[3][2],ax[3][3]],fraction=0.047, pad = 0,orientation = 'horizontal',label='FSYSC (g C m$^-$$^2$)',extend='both')
plt.savefig(file_path + "FSYSC/2035_2050/2035_2050.jpg",dpi = 1000)
...
plt.axis('off')
plt.colorbar(im,fraction = 0.05, pad = -0.01,orientation = 'horizontal',label='FSYSC (g C m$^-$$^2$ yr$^-$$^1$)',extend='both')
plt.savefig(file_path + "FSYSC/2035_2050/even_35_50.jpg",dpi = 1000)
